...
import type { ServicoExtra } from "../../types/servicosExtras"
  ...
  return (
    <div className="max-w-6xl mx-auto p-4">
      <div className="mb-6">
        ...
        <p className="text-gray-600">Adicione, edite ou remova serviços extras que aparecerão nos orçamentos</p>
      </div>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
          ...
          <form onSubmit={editando ? atualizarServico : salvarServico} className="space-y-4">
            <div>
              ...
              <input
                ...
                placeholder="Ex: Garantia estendida, Compressor novo, etc."
                required
              />
            </div>
            <div>
        ...
        {/* Lista de serviços */}
        <div className="bg-white rounded-lg shadow p-6">
          <h2 className="text-lg font-semibold mb-4">📋 Serviços Extras Cadastrados</h2>
          <div className="space-y-2 max-h-[500px] overflow-y-auto">
            {servicos.length === 0 ? (
              <p className="text-gray-500 text-center py-8">Nenhum serviço extra cadastrado</p>
            ) : (
              servicos.map((servico) => (
                <div key={servico.id} className={`border rounded-lg p-3 ${!servico.ativo ? "bg-gray-50 opacity-60" : ""}`}>
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      <div className="flex items-center gap-2">
                        <h3 className="font-semibold">{servico.nome}</h3>
                        {!servico.ativo && (
                          <span className="text-xs bg-gray-300 text-gray-600 px-2 py-0.5 rounded">Inativo</span>
                        )}
                      </div>
                      {servico.descricao && (
                        <p className="text-sm text-gray-500 mt-1">{servico.descricao}</p>
                      )}
                      <p className="text-lg font-bold text-green-600 mt-1">{formatCurrency(servico.valor)}</p>
                    </div>
                    <div className="flex gap-2">
                      <button
