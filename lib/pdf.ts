...
export async function saveQuotePdf(input: QuotePdfInput, filename: string) {
  ...
  // ============================================
  // GERAR HTML DO PDF (TAMANHO INTERMEDIÁRIO)
  // ============================================
    ...
    for (const line of itensUnicos) {
      ...
      tabelaHTML += `
        <tr style="background: ${bgColor}; border-bottom: 1px solid #e5e7eb;">
          <td style="padding: 6px 10px; text-align: left; font-size: 12px; color: #1a1a1a; max-width: 300px; word-wrap: break-word;">${line.descricao}</td>
          <td style="padding: 6px 10px; text-align: center; font-size: 12px; color: #1a1a1a;">${line.quantidade}</td>
          <td style="padding: 6px 10px; text-align: right; font-size: 12px; color: #1a1a1a;">${money(line.unitario)}</td>
          <td style="padding: 6px 10px; text-align: right; font-size: 12px; font-weight: bold; color: #1a1a1a;">${money(line.total)}</td>
        </tr>
      `
      linhaCount++
    }
  ...
  if (servicosExtrasLista.length > 0) {
    ...
    for (const servico of servicosUnicos) {
      servicosHTML += `<li style="margin: 1px 0; font-size: 11px; color: #4a4a4a; list-style: none; padding-left: 16px;">• ${servico}</li>`
    }
  }
  ...
  if (observacoesText && observacoesText.trim() !== "") {
    observacoesHTML = `
      <div style="background: #eff6ff; padding: 10px 14px; border-radius: 8px; margin-top: 10px;">
        <p style="font-weight: bold; font-size: 11px; color: #1a2a4f; margin: 0 0 5px 0;">OBSERVAÇÕES</p>
        <p style="font-size: 11px; color: #333333; margin: 0;">${observacoesText}</p>
      </div>
    `
  }

  // 🔧 TAMANHO INTERMEDIÁRIO (nem muito grande, nem muito pequeno)
  element.innerHTML = `
    <!-- HEADER -->
    ...
    <!-- TÍTULO -->
    <div style="text-align: center; padding: 18px 0; border-bottom: 2px solid #c9a03d;">
      ...
      ${input.numero ? `<p style="color: #64748b; font-size: 12px; margin: 3px 0 0 0;">Número: ${input.numero}</p>` : ""}
    </div>
    
    <!-- CLIENTE -->
    <div style="background: #f8fafc; padding: 10px 16px; border-radius: 6px; margin: 12px 0; border: 1px solid #e5e7eb;">
      <p style="font-weight: bold; font-size: 11px; color: #1a2a4f; margin: 0 0 4px 0;">CLIENTE</p>
      <p style="font-size: 15px; color: #1a1a1a; margin: 0;">${input.cliente}</p>
    </div>
    
    <!-- STATUS E SERVIÇOS EXTRAS -->
    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 10px; margin: 10px 0;">
      <div style="background: ${statusColor}; padding: 10px 14px; border-radius: 6px;">
        <p style="font-weight: bold; font-size: 10px; color: #1a2a4f; margin: 0 0 4px 0;">STATUS</p>
        <p style="font-size: 14px; color: #1a1a1a; margin: 0;">${statusText}</p>
      </div>
      ${servicosExtrasTotal > 0 ? `
        <div style="background: #fef3c7; padding: 10px 14px; border-radius: 6px;">
          <p style="font-weight: bold; font-size: 10px; color: #1a2a4f; margin: 0 0 4px 0;">SERVIÇOS EXTRAS</p>
    ...
    ${servicosHTML ? `
      <div style="background: #fffbeb; padding: 10px 16px; border-radius: 6px; margin: 8px 0;">
        <p style="font-weight: bold; font-size: 11px; color: #1a2a4f; margin: 0 0 6px 0;">SERVIÇOS CONTRATADOS:</p>
      ...
      <div style="margin-top: 12px; border: 1px solid #e5e7eb; border-radius: 6px; overflow: hidden;">
        <table style="width: 100%; border-collapse: collapse; font-size: 11px;">
          <thead style="background: #1a2a4f; color: white;">
            <tr>
              <th style="padding: 6px 10px; text-align: left; font-size: 11px; font-weight: bold;">DESCRIÇÃO</th>
              <th style="padding: 6px 10px; text-align: center; font-size: 11px; font-weight: bold;">QTD</th>
              <th style="padding: 6px 10px; text-align: right; font-size: 11px; font-weight: bold;">UNITÁRIO</th>
              <th style="padding: 6px 10px; text-align: right; font-size: 11px; font-weight: bold;">TOTAL</th>
            </tr>
          </thead>
          <tbody>
            ${tabelaHTML}
    ...
    <!-- TOTAL GERAL -->
    <div style="background: linear-gradient(135deg, #c9a03d 0%, #b58d2c 100%); text-align: center; padding: 14px 18px; border-radius: 8px; margin: 12px 0 0 0; box-shadow: 0 3px 12px rgba(201, 160, 61, 0.3);">
      <p style="color: #1a2a4f; font-size: 14px; font-weight: bold; margin: 0 0 4px 0; letter-spacing: 2px;">TOTAL GERAL</p>
      <p style="font-size: 30px; font-weight: bold; color: white; margin: 0; text-shadow: 0 2px 4px rgba(0,0,0,0.2);">${money(input.total)}</p>
    </div>
  `
