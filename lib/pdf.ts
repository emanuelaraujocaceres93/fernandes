...
export async function saveQuotePdf(input: QuotePdfInput, filename: string) {
  ...
  element.innerHTML = `
    <!-- HEADER -->
    <div style="text-align: center; padding: 25px 20px; background: linear-gradient(135deg, #1a2a4f 0%, #2c3e66 100%); border-radius: 10px 10px 0 0;">
      ${input.config?.logo_url ? `<img src="${input.config.logo_url}" style="max-width: 120px; margin: 0 auto 12px auto; display: block;" />` : ""}
      <h1 style="color: white; margin: 0; font-size: 22px; letter-spacing: 2px;">${input.config?.nome_empresa || "Fernandes Sistemas"}</h1>
      ${input.config?.telefone ? `<p style="color: #c9a03d; margin: 6px 0 0 0; font-size: 13px;">📞 ${input.config.telefone}</p>` : ""}
    </div>
    ...
    <!-- CLIENTE -->
    <div style="background: #f8fafc; padding: 10px 14px; border-radius: 6px; margin: 12px 0; border: 1px solid #e5e7eb;">
    ...
    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 10px; margin: 10px 0;">
      <div style="background: ${statusColor}; padding: 8px 12px; border-radius: 6px;">
        <p style="font-weight: bold; font-size: 9px; color: #1a2a4f; margin: 0 0 4px 0;">STATUS</p>
      ...
      ${servicosExtrasTotal > 0 ? `
        <div style="background: #fef3c7; padding: 8px 12px; border-radius: 6px;">
          <p style="font-weight: bold; font-size: 9px; color: #1a2a4f; margin: 0 0 4px 0;">SERVIÇOS EXTRAS</p>
    ...
    ${servicosHTML ? `
      <div style="background: #fffbeb; padding: 8px 14px; border-radius: 6px; margin: 8px 0;">
        <p style="font-weight: bold; font-size: 11px; color: #1a2a4f; margin: 0 0 5px 0;">SERVIÇOS CONTRATADOS:</p>
    ...
    <!-- TOTAL GERAL -->
    <div style="background: linear-gradient(135deg, #c9a03d 0%, #b58d2c 100%); text-align: center; padding: 12px 16px; border-radius: 8px; margin: 12px 0; box-shadow: 0 3px 12px rgba(201, 160, 61, 0.3);">
  ...
  // 🔧 ALTURA ÚTIL DA PÁGINA (A4 = 297mm)
